// Copyright 2015 The Prometheus Authors
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
// http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package graphite

import (
	"bytes"
	"fmt"
	"math"
	"net"
	//"sort"
	"time"

	"github.com/prometheus/common/log"
	"github.com/prometheus/common/model"

	"strings"
)

const NameSpace  = "PAAS"

// Client allows sending batches of Prometheus samples to Graphite.
type Client struct {
	address   string
	transport string
	timeout   time.Duration
	prefix    string
}

// NewClient creates a new Client.
func NewClient(address string, transport string, timeout time.Duration, prefix string) *Client {
	return &Client{
		address:   address,
		transport: transport,
		timeout:   timeout,
		prefix:    prefix,
	}
}

func pathFromMetric(m model.Metric, prefix string) string {
	var buffer bytes.Buffer

	buffer.WriteString(prefix)


	buffer.WriteString(escape(m[model.MetricNameLabel]))


	// filter unused metric labels
	mfilter, labels := LabelsFilter(m)
	log.Debugf(" remote storage adapter filtering labels...")

	// We want to sort the labels.
	//labels := make(model.LabelNames, 0, len(mfilter))

	// 按照 const 中定义的 label 顺序进行 map 的顺序输出，否则在 graphite 侧无法保证目录层次的构造
	//labels := make(model.LabelNames, 0, len(mlabels))
	//for l := range mlabels {
	//	labels = append(labels, l)
	//}
	// 不要排序，按照 自定义的 label 顺序进行graphite目录组织，否则无法构造标准化请求
	//sort.Sort(labels)

	// For each label, in order, add ".<label>.<value>".
	for _, l := range labels {
		v := mfilter[l]

		if l == model.MetricNameLabel || len(l) == 0 {
			continue
		}
		// Since we use '.' instead of '=' to separate label and values
		// it means that we can't have an '.' in the metric name. Fortunately
		// this is prohibited in prometheus metrics.


		buffer.WriteString(fmt.Sprintf(
			".%s.%s", string(l), escape(v)))
	}

	return buffer.String()
}

// Write sends a batch of samples to Graphite.
func (c *Client) Write(samples model.Samples) error {
	conn, err := net.DialTimeout(c.transport, c.address, c.timeout)
	if err != nil {
		return err
	}
	defer conn.Close()

	var buf bytes.Buffer
	for _, s := range samples {
		k := pathFromMetric(s.Metric, c.prefix)
		// 规范 graphite 数据库存储数据时组织目录, eg: PAAS.node.noede_memory_MemTotal.instance,其中 NODE 为一级目录 subnamespace
		sn := strings.Split(k, "_")[0]

		t := float64(s.Timestamp.UnixNano()) / 1e9
		v := float64(s.Value)
		if math.IsNaN(v) || math.IsInf(v, 0) {
			log.Debugf("cannot send value %f to Graphite,"+
				"skipping sample %#v", v, s)
			continue
		}

		// graphite 通过 "." 来切分目录层次，在添加一级目录的时候为达到预期的目录组织需要自己加上此拼接符
		fmt.Fprintf(&buf, "%s.%s.%s %f %f\n", NameSpace, sn, k, v, t)
	}

	_, err = conn.Write(buf.Bytes())
	if err != nil {
		return err
	}

	return nil
}

// Name identifies the client as a Graphite client.
func (c Client) Name() string {
	return "graphite"
}
